...
$users = $sql->query("SELECT ".userfields('u').",u.posts,u.joined,COUNT(*) num FROM users u LEFT JOIN posts p ON p.user = u.id WHERE p.date > ? GROUP BY u.id ORDER BY num DESC",
	[(time() - $time)]);
?>
...
for ($i = 1; $user = $users->fetch(); $i++) {
	...
	?><tr class="n<?=$tr ?> center">
		...
		<td class="b"><?=dateformat($user['joined']) ?></td>
